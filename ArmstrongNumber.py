...
def Armstrong_Num():
    ...
    if temp == sum:
          messagebox.showinfo("ARMSTRONG_NUM"," It is an armstrong no")
    else:
          messagebox.showinfo("ARMSTRONG_NUM","It is not armstrong no")
    window = Tk()
    window.title("ARMSTRONG_NUMBER")
    blabel = Label(window)
    blabel.grid()
    frame=Frame(window)
    ...
    num.grid(row=0, column=0, padx=10,pady=5)
